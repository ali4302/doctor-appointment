...
export default function AdminLogin() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      await loginAsAdmin(form.username, form.password)
      navigate('/admin')
    ...
    <div className="min-h-screen bg-slate-800 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md fade-in">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <Input label="Username" placeholder="admin" icon={User} value={form.username} onChange={e=>setForm({...form,username:e.target.value})} required/>
